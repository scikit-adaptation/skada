...
* `pack` masks labels for domains designated for being used as targets (default argument `mask_target_labels=True`)
* `pack` with argument `mask_target_labels=False` packs requested targets
