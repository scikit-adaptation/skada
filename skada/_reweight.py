...
from .metrics import MaximumMeanDiscrepancy
from .params import DirectWeighting
from .optims import projected_gradient_descent, frank_wolfe
...
class KMM(BaseDataAdaptEstimator):
    ...
    coef0 : int 
        Parameters for the kernels.
    algo : str, default="frank-wolfe"
        Optimization algorithm, "frank-wolfe" or "pgd"
    lr : float, default=0.001
        Learning rate of the gradient ascent (for algo="pgd").
    ...
    def predict_adapt(self, X, y, X_target, y_target=None):
        ...
        weights : array-like, shape (n_samples,)
            The weights of the samples.
        """
        weights = self.weight_param_.predict("src")
        return X, y, weights
    ...
    def fit_adapt(self, X, y, X_target, y_target=None):
        ...
        metric.fit(X, X_target, Ws=weight_param.predict())
        
        if self.algo == "frank-wolfe":
            optim_func = frank_wolfe
            optim_params = dict(max_iter=self.max_iter, verbose=self.verbose)
        elif self.algo == "pgd":
            optim_func = projected_gradient_descent
            optim_params = dict(lr=self.lr, max_iter=self.max_iter, verbose=self.verbose)
        ...
        self.weight_param_ = weight_param
