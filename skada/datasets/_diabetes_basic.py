...
def read_dataset(data_folder):
    ...
    ids_mapping_data = read_csv(os.path.join(data_folder, 'IDS_mapping.csv'), header=False)
    diabetic_data_data = read_csv_with_header(os.path.join(data_folder, 'diabetic_data.csv'))

    # Identify the indices where sections change
    section_indices = [i for i, row in enumerate(ids_mapping_data) if all(val == '' for val in row)]
...
def preprocess_dataset(
    ...
    columns_to_drop = ['encounter_id', 'patient_nbr', 'max_glu_serum', 'A1Cresult', 'weight', 'payer_code', 'medical_specialty', 'diag_1', 'diag_2', 'diag_3']
    diabetic_data_data = [{k: v for k, v in entry.items() if k not in columns_to_drop} for entry in diabetic_data_data]

    # Drop rows with 'Unknown/Invalid' value (only 3 rows)
    diabetic_data_data = [entry for entry in diabetic_data_data if entry['gender'] != 'Unknown/Invalid']
...
def generate_domain_aware_dataset(diabetic_data_data):
    ...
    for domain_name in grouped_data.keys():
        ...
        #race_data_X = [entry for entry in race_data for key, value in entry.items() if key != target_key]
        #X = np.array([list(entry.values()) for entry in race_data_X])

        #y = np.array([int(row[target_key]) for row in race_data])

        # race_data_X = [entry for entry in race_data if target_key not in entry]
        # X = np.array([list(entry.values()) for entry in race_data_X])
...
def fetch_diabetes_dataset(only_domain_aware=False):
    # URL of the dataset
    dataset_url = """https://archive.ics.uci.edu/
    static/public/296/diabetes+130-us+hospitals+for+years+1999-2008.zip"""
    ...
    (admission_type_data, discharge_disposition_data,
        admission_source_data, diabetic_data_data) = read_dataset(destination_folder)
    ...
    dataset = generate_domain_aware_dataset(diabetic_data_data)

    if only_domain_aware:
        return dataset

    return (dataset, admission_type_data, discharge_disposition_data,
            admission_source_data, diabetic_data_data)
...
#     ids_mapping_df = pd.read_csv(
#         os.path.join(data_folder, 'IDS_mapping.csv'),
...
#     sections = [
#         ids_mapping_df.iloc[section_indices[i]+1:section_indices[i+1]]
#         for i in range(len(section_indices)-1)
#     ]
#     sections = [section.reset_index() for section in sections]
#     sections = [section.drop(columns=['index']) for section in sections]
#     sections = [
#         section.rename(columns=section.iloc[0]).drop(section.index[0])
...
#     return (admission_type_df, discharge_disposition_df,
#             admission_source_df, diabetic_data_df)
...
#     For the Diabetes prediction task, we use a set of features related to
#     several known indicators for diabetes derived from. These risk factors
#     are general physical health, high cholesterol, BMI/obesity, smoking,
#     the presence of other chronic health conditions (stroke,
#     coronary heart disease), diet, alcohol consumption, exercise, household
#     income, marital status, time since last checkup, education level,
#     healthcare coverage, and mental health.
#     For each risk factor, we extract a set of relevant features from the
#     BRFSS foxed core and rotating core questionnaires. We also use a shared
#     set of demographic indicators (race, sex, state, survey year, and a
#     question related to income level).
#     The prediction target is a binary indicator for whether the respondent has ever
...
#     diabetic_data_df.loc[:, 'readmitted'] = diabetic_data_df['readmitted'].apply(
#         lambda x: 0 if x == 'NO' else 1
#         )

#     # Drop rows with 'Unknown/Invalid' value (only 3 rows)
#     diabetic_data_df = diabetic_data_df.loc[
#         diabetic_data_df['gender'] != 'Unknown/Invalid'
#     ]

#     # Convert 'gender' to binary
#     diabetic_data_df.loc[:, 'gender'] = diabetic_data_df['gender'].map(
#         {'Female': 0, 'Male': 1}
#         )
...
#         y = race_df['readmitted'].values.astype(int)
...
#     (admission_type_df, discharge_disposition_df,
#         admission_source_df, diabetic_data_df) = read_dataset(destination_folder)

#     # Preprocess the dataset
#     (admission_type_df, discharge_disposition_df,
#         admission_source_df, diabetic_data_df) = preprocess_dataset(
#         admission_type_df, discharge_disposition_df,
#         admission_source_df, diabetic_data_df
#             )
...
#     return (dataset, admission_type_df, discharge_disposition_df,
#             admission_source_df, diabetic_data_df)