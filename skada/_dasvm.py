...
class BaseDasvmAdapter(BaseAdapter, BaseSelector):
    ...
    """
    def __init__(self, base_estimator: BaseEstimator = SVC(gamma='auto'), k = 3, Stop = 1_000, **kwargs):
    ...
    def _find_points_next_step(self, I_list, d, c):
        ...
        # We should take k points for each of the c classes, depending on the values of d
        for j in range(min(self.k, math.ceil(d.shape[0]/c))):
            mask = np.ones(d.shape[0], dtype=bool)
            mask[I] = False
            I_ = np.unique(np.argmax(d[mask], axis=0))
            # We need to get all those indices to be take into account the fact that the some previous points weren't in the list
            for l in I:
                I_[I_>=l]+=1
            I = np.concatenate((I, I_))
            I = np.sort(I) # we sort I as it is an assumption for I to be sorted for the previous algorithm
    
        # Again we need to get all those indices to be take into account the fact that the some previous points weren't in the list
    ...
    def fit(self, X, y=None, sample_domain=None):
        ...
        # This is the list of the indices from the points from Xs that have been discarded
        Id = [np.array([False]*n)]
        # This is the list of the indices from the points from Xt that have been added
        Ia = [np.array([False]*m)]
        ...
        # We need to look at the decision function for to select the labaled data that we discard and the semi-labeled points that we will add
        ...
        while (sum(Ia[-1])<m or sum(Id[-1])<n) and i<self.Stop:
            ...
            # We want to take values that are unsure, meaning we want those that have 
            # values the closest that we can to c-1 (to 0 when label='binary', or 4 when is its 'multiclass')
            decisions_ta = -np.abs(decisions_ta-c-1)
        ...
        # t could be interesting to return multiple estimators, or the list Ia and Id (this making them being an object attribute)
        return self
