...
def _estimator_has(attr, base_attr_name="base_estimator"):
    ...
    def has_base_estimator(estimator) -> bool:
        return hasattr(estimator, base_attr_name) and hasattr(
            getattr(estimator, base_attr_name), attr
        )
    ...
    def has_estimator_selector(estimator) -> bool:
        return hasattr(estimator, "estimators_") and hasattr(
            estimator.estimators_[0], attr
        )

    return lambda estimator: (
        has_base_estimator(estimator) or has_estimator_selector(estimator)
    )
...
class BaseAdapter(BaseEstimator):
    ...
    @abstractmethod
    def adapt(
        self, X, y=None, sample_domain=None, **params
    ...
    def fit_transform(self, X, y=None, sample_domain=None, **params):
        ...
        return self.transform(
            X, y=y, sample_domain=sample_domain, allow_source=True, **params
        )
...
class BaseSelector(BaseEstimator, _DAMetadataRequesterMixin):
    ...
    def _route_and_merge_params(self, routing_request, X_input, params):
        ...
        X_out, sample_domain = check_X_domain(
            X_out, sample_domain=params.get("sample_domain")
        )
...
class Shared(BaseSelector):
    ...
    def _route_to_estimator(self, method_name, X, y=None, **params):
        ...
        output = (
            method(X, **routed_params) if y is None else method(X, y, **routed_params)
        )
        return output
...
class PerDomain(BaseSelector):
    ...
    def _route_to_estimator(self, method_name, X, y=None, **params):
        ...
        for domain_label in np.unique(sample_domain):
            ...
            if output is None:
                output = np.zeros(
                    (X.shape[0], *domain_output.shape[1:]), dtype=domain_output.dtype
                )
    ...
    def fit(self, X, y, **params):
        if y is not None:
            X, y, sample_domain = check_X_y_domain(
                X, y, sample_domain=params.get("sample_domain")
            )
        else:
            X, sample_domain = check_X_domain(
                X, sample_domain=params.get("sample_domain")
            )
        params["sample_domain"] = sample_domain
...
class SelectSourceTarget(BaseSelector):
    def __init__(
        self,
        source_estimator: BaseEstimator,
        target_estimator: Optional[BaseEstimator] = None,
    ):
        if (
            target_estimator is not None
            and hasattr(source_estimator, "transform")
            and not hasattr(target_estimator, "transform")
        ):
            raise TypeError(
                "The provided source and target estimators must "
    ...
    def get_metadata_routing(self):
        ...
        routing.add(
            estimator=self.source_estimator,
            method_mapping=MethodMapping()
            .add(callee="fit", caller="fit")
            .add(callee="partial_fit", caller="partial_fit")
            .add(callee="transform", caller="transform")
    ...
    def fit(self, X, y, **params):
        ...
        for domain_type, base_estimator, domain_masks in [
            ...
            X_masked, y_masked, params_masked = _apply_domain_masks(
                X, y, params, masks=domain_masks
            )
            routing = get_routing_for_object(base_estimator)
            X_masked, routed_params = self._route_and_merge_params(
                routing.fit, X_masked, params_masked
            )
            X_masked, y_masked, routed_params = self._remove_masked(
                X_masked, y_masked, routed_params
            )
    ...
    def _route_to_estimator(self, method_name, X, y=None, **params):
        ...
        for domain_estimator, domain_masks in [
            ...
            if domain_masks.sum() == 0:
                ...
                continue
            X_domain, y_domain, params_domain = _apply_domain_masks(
                X, y, params, masks=domain_masks
            )
            request = getattr(get_routing_for_object(domain_estimator), method_name)
            X_domain, routed_params = self._route_and_merge_params(
                request, X_domain, params_domain
            )
    ...
    @available_if(
        _estimator_has("predict_log_proba", base_attr_name="source_estimator")
    )
    def predict_log_proba(self, X, **params):
        return self._route_to_estimator("predict_log_proba", X, **params)

    @available_if(
        _estimator_has("decision_function", base_attr_name="source_estimator")
    )
    def decision_function(self, X, **params):
        return self._route_to_estimator("decision_function", X, **params)
