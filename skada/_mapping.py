...
class LinearWassersteinBarycenterTransportAdapter(BaseAdapter):
    ...
    ----------
    reg : float, default=1e-08
        Regularization parameter to ensure numerical stability in the
        computation of the barycenter and mappings.
    bias : bool, default=True
        If True, include a bias term in the affine mapping. If False,
        the mapping is restricted to a linear transformation.
    test_time : bool, default=False
        If True, enables test-time adaptation behavior. If False, the
        model operates in training mode.

    Attributes
    ----------
    cov_means_sources_ : dict
        A dictionary containing the covariance matrices and mean vectors
        for each source domain.
    cov_means_targets_ : dict
        A dictionary containing the covariance matrices and mean vectors
        for each target domain.
    barycenter_ : tuple
        A tuple containing the mean vector and covariance matrix of the
        estimated barycentric measure.
    mappings_ : dict
        A dictionary containing the affine mappings (A, b) for each
        source domain to the barycenter.
    mappings_target_ : dict
        A dictionary containing the affine mappings (A, b) from the
        barycenter to each target domain.
    ...
    def fit(self, X, y=None, *, sample_domain=None):
        ...
        C = np.stack([cov for cov, _ in self.cov_means_sources_.values()])
        m = np.stack([mean for _, mean in self.cov_means_sources_.values()])
        ...
        # Mapping of each source domain towards the barycenter
        self.mappings_ = {
            domain: bures_wasserstein_mapping(
                ms=mean,
                mt=self.barycenter_[0],
                Cs=cov,
                Ct=self.barycenter_[1],
            )
            for domain, (cov, mean) in self.cov_means_sources_.items()
        }

        # Mapping of the barycenter towards thet target domain
        self.mappings_target_ = {
            domain: bures_wasserstein_mapping(
                mt=mean,
                ms=self.barycenter_[0],
                Ct=cov,
                Cs=self.barycenter_[1],
            )
            for domain, (cov, mean) in self.cov_means_targets_.items()
        }
    ...
    def transform(
        ...
        for domain in source_domains:
            A, b = self.mappings_[domain]
            _X, _y = source_domains[domain][0], source_domains[domain][1]

            # Appends mapped samples to the barycenter measure support
            est_XB.append(_X.dot(A) + b)
            est_yB.append(_y)
        est_XB = np.concatenate(est_XB, axis=0)
        est_yB = np.concatenate(est_yB, axis=0)
